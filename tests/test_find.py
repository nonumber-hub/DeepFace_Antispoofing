...
def test_find_with_exact_path():
    img_path = os.path.join("tests/dataset", "img1.jpg")
...
def test_filetype_for_find():
    ...
    assert df[df["identity"] == "tests/dataset/img47.jpg"].shape[0] == 0
...
def test_filetype_for_find_bulk_embeddings():
    ...
    # img47 is webp even though its extension is jpg
    assert "tests/dataset/img47.jpg" not in list_imgs
    ...
    # img47 is webp even though its extension is jpg
    assert "tests/dataset/img47.jpg" not in gen_imgs
...
def test_find_without_refresh_database():
    ...
    pkl_path = "tests/dataset/ds_model_vggface_detector_opencv_aligned_normalization_base_expand_0.pkl"
    with open(pkl_path, "rb") as f:
        hash_before = hashlib.sha256(f.read())

    image_name = "img28.jpg"
    tmp_dir = "tests/dataset/temp_image"
    os.mkdir(tmp_dir)
    shutil.move(os.path.join("tests/dataset", image_name), os.path.join(tmp_dir, image_name))

    dfs = DeepFace.find(img_path=img_path, db_path="tests/dataset", silent=True, refresh_database=False)

    with open(pkl_path, "rb") as f:
        hash_after = hashlib.sha256(f.read())

    shutil.move(os.path.join(tmp_dir, image_name), os.path.join("tests/dataset", image_name))
    os.rmdir(tmp_dir)
